...
if ($emo == 'saldumi'){
	$vards = 'Tauki, saldumi';
	$vardi = mysql_query("SELECT tvits FROM words where grupa = '1' limit 0, 500");
}else if ($emo == 'gala'){
	$vards = 'Gaļa, olas, zivis';
	$vardi = mysql_query("SELECT tvits FROM words where grupa = '2' limit 0, 500");
}else if ($emo == 'piens'){
	$vards = 'Piena produkti';
	$vardi = mysql_query("SELECT tvits FROM words where grupa = '3' limit 0, 500");
}else if ($emo == 'darzeni'){
	$vards = 'Dārzeņi';
	$vardi = mysql_query("SELECT tvits FROM words where grupa = '4' limit 0, 500");
}else if ($emo == 'augli'){
	$vards = 'Augļi, ogas';
	$vardi = mysql_query("SELECT tvits FROM words where grupa = '5' limit 0, 500");
}else if ($emo == 'maize'){
	$vards = 'Maize, graudaugu produkti, makaroni, rīsi, biezputras, kartupeļi';
	$vardi = mysql_query("SELECT tvits FROM words where grupa = '6' limit 0, 500");
}else if ($emo == 'alkoholiskie'){
	$vards = 'Alkoholiskie dzērieni';
	$vardi = mysql_query("SELECT tvits FROM words where grupa = '7' limit 0, 500");
}else if ($emo == 'bezalkoholiskie'){
	$vards = 'Bezalkoholiskie dzērieni';
	$vardi = mysql_query("SELECT tvits FROM words where grupa = '8' limit 0, 500");
}else if ($emo == 'pozitivi'){
	$vards = 'Pozitīvie';
	$vardi = mysql_query("SELECT text, screen_name FROM tweets where emo = '1' group by created_at desc limit 0, 500");
}else if ($emo == 'negativi'){
	$vards = 'Negatīvie';
	$vardi = mysql_query("SELECT text, screen_name FROM tweets where emo = '2' group by created_at desc limit 0, 500");
}else if ($emo == 'neitrali'){
	$vards = 'Neitrālie';
	$vardi = mysql_query("SELECT text, screen_name FROM tweets where emo = '3' group by created_at desc limit 0, 500");
}else if ($emo == 'Mon'){
	$vards = 'Pirmdienas';
